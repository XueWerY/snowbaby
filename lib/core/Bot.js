...
export default class Yunzai extends EventEmitter {
  ...
  wsConnect(req, socket, head) {
    ...
    this.wss.handleUpgrade(req, socket, head, conn => {
      conn.on("error", (...args) => util.makeLog("error", args, "Bot", true))
      conn.on("close", () => util.makeLog("mark", "断开连接", "Bot", true))
      conn.on("message", msg =>
        util.makeLog("debug", ["消息", util.String(msg)], "Bot", true),
      )
      conn.sendMsg = msg => {
        if (!Buffer.isBuffer(msg)) msg = util.String(msg)
        util.makeLog("debug", ["消息", msg], "Bot", true)
