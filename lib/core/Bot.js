...
export default class Yunzai extends EventEmitter {
  ...
  express = Object.assign(express(), { skip_auth: [], quiet: [] })
    ...
    .use("/bots", this.serverBots.bind(this))
    .use("/config", this.serverConfig.bind(this))
  ...
  serverBots(req) {
    ...
    for (const bot_id of this.uin) {
      ...
      bots.push({
        ...
        version: b.version,
        clients: b.clients,
        online: b.stat?.online,
        friend_count: b.fl?.size || 0,
        group_count: b.gl?.size || 0,
      })
    }
    req.res.type("json")
    req.res.send(
      JSON.stringify({
        total: bots.length,
        friend_count: this.getFriendMap().size,
        group_count: this.getGroupMap().size,
        bots,
      }),
    )
  }
  ...
  serverConfig(req) {
    // GET：返回合并后配置（默认 + 用户，敏感字段 auth 脱敏）
    if (req.method === "GET") {
      const merged = {}
      for (const name in cfg.defaults || {}) {
        merged[name] = cfg.getAllCfg(name)
      }
      // 脱敏鉴权信息，避免泄露
      if (merged.server?.auth)
        merged.server = { ...merged.server, auth: Object.fromEntries(Object.keys(merged.server.auth).map(k => [k, "******"])) }
      req.res.type("json")
      req.res.send(
        JSON.stringify({
          user: cfg.getConfig(),
    ...
    // PATCH：部分覆盖用户配置并热重载
    if (req.method === "PATCH" || (req.method === "POST" && req.query._method === "PATCH")) {
      ...
      cfg
        ...
        .catch(err => {
          req.res.status(500).type("json").send(JSON.stringify({ ok: false, error: String(err) }))
        })
      return
    }

    req.res.sendStatus(405)
  }
  ...
  wsConnect(req, socket, head) {
    ...
    this.wss.handleUpgrade(req, socket, head, conn => {
      conn.on("error", (...args) => util.makeLog("error", args, "WebSocket", true))
      conn.on("close", () => util.makeLog("mark", "断开连接", "WebSocket", true))
      conn.on("message", msg =>
        util.makeLog("debug", ["消息", util.String(msg)], "WebSocket", true),
      )
      conn.sendMsg = msg => {
        if (!Buffer.isBuffer(msg)) msg = util.String(msg)
        util.makeLog("debug", ["消息", msg], "WebSocket", true)
  ...
  async serverLoad(https) {
    ...
    const displayAddr = address === "::" ? "127.0.0.1" : address
    util.makeLog(
      "mark",
      [
        `启动 HTTP${https ? "S" : ""} 服务器`,
        logger.green(`http${https ? "s" : ""}://${displayAddr}:${port}`),
      ],
      "Server",
    )
  ...
  async run() {
    ...
    util.makeLog(
      "info",
      `onebotv11 WS 反向客户端连接地址：${logger.blue(this.url.replace(/^http/, "ws") + "/" + Object.keys(this.wsf)[0] + "/v11")}`,
      "WebSocket",
    )
  ...
  async fileToUrl(file, opts = {}) {
    const {
      name,
      time = cfg.bot.fileToUrlTime * 60000,
      times = cfg.bot.fileToUrlTimes,
    } = opts
