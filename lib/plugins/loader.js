...
class PluginsLoader {
  ...
  async load(isRefresh = false) {
    ...
    await Promise.allSettled(
      files.map(async file => {
        if (
          (await Bot.sleep(
            cfg.bot.pluginLoadTimeout * 1000,
  ...
  /** 初始化插件规则正则 */
  initReg(rule) {
    for (const i of rule) if (!(i.reg instanceof RegExp)) i.reg = new RegExp(i.reg)
  }
  ...
  async loadPlugin(file, p) {
    ...
    plugin = new p()
    /** 初始化正则表达式 */
    if (plugin.rule) this.initReg(plugin.rule)
  ...
  async saveCount(e, type) {
    ...
    for (const i of key) await localStore.incr(`snowbaby:count:${type}:${i}`)
  }
