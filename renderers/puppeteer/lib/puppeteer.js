...
export default class Puppeteer extends Renderer {
  constructor() {
    ...
    /** 截图次数 */
    this.renderNum = 0
    // 本地 json 覆盖配置（data/<用户ID>/snowbaby/puppeteer.json）
    let localConfig = {}
    try {
      localConfig =
    ...
    if (localConfig.chromiumPath || cfg?.puppeteer?.chromiumPath || cfg?.bot?.chromiumPath)
      /** chromium其他路径 */
      this.config.executablePath =
        localConfig.chromiumPath || cfg?.puppeteer?.chromiumPath || cfg?.bot?.chromiumPath
    if (localConfig.puppeteerWS || cfg?.puppeteer?.puppeteerWS || cfg?.bot?.puppeteerWs)
      /** chromium其他路径 */
      this.config.wsEndpoint =
        localConfig.puppeteerWS || cfg?.puppeteer?.puppeteerWS || cfg?.bot?.puppeteerWs
    /** puppeteer超时超时时间 */
    this.puppeteerTimeout =
      localConfig.puppeteerTimeout ||
      cfg?.puppeteer?.puppeteerTimeout ||
      cfg?.bot?.puppeteerTimeout ||
      0
    this.pageGotoParams = localConfig.pageGotoParams || cfg?.puppeteer?.pageGotoParams || {
      timeout: 120000,
  ...
  async browserInit() {
    ...
    try {
      ...
      if (!mac) {
        ...
        this.browserMacKey = `snowbaby:chromium:browserWSEndpoint:${mac}`
      }
      // 是否有browser实例
      const browserUrl = (await localStore.get(this.browserMacKey)) || this.config.wsEndpoint
      if (browserUrl) {
        try {
          ...
          await localStore.del(this.browserMacKey)
        }
      }
    } catch {}
    ...
    if (!connectFlag) {
      ...
      if (this.browserMacKey) {
        ...
        await localStore.set(this.browserMacKey, this.browser.wsEndpoint(), { EX: expireTime })
      }
    }
