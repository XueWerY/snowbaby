...
class Util {
  makeLogID(id) {
    if (!id) return cfg.bot.logAlign || "snowbaby"
    if (typeof id !== "string") id = this.String(id)
    return id
  }

  makeLog(level, msg, id, force) {
    const head = logger.head(level, id === false ? "" : force ? id : this.makeLogID(id))
    const body = (Array.isArray(msg) ? msg : [msg]).map(i => this.Loging(i)).join(" ")
    logger.logger[level](head + " " + body)
  }
  ...
  Loging(data, opts = cfg.bot.logObject) {
