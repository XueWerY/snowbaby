...
export class Status extends Plugin {
  ...
  async status() {
    ...
    const msg =
      `—— snowbaby v${cfg.package.version} ——\n` +
  ...
  botTime() {
    ...
    for (const i of Bot.uin)
      if (Bot[i]?.stat?.start_time)
        lines.push(`${Bot.getTimeDiff(Bot[i].stat.start_time * 1000)} ${i}`)
    return `账号在线时长${lines.map(line => `\n${line}`).join("")}`
  }

  pluginTime() {
    const lines = []
    for (const i in PluginsLoader.load_time)
      lines.push(`${Bot.getTimeDiff(0, PluginsLoader.load_time[i])} ${i}`)
    return `插件加载用时${lines.map(line => `\n${line}`).join("")}`
  }
  ...
  async getCount(cmd) {
    ...
    for (const i of array) {
      ...
      for (let d of date) {
        ...
        const ret = await this.count(i.type, key)
  ...
  async count(type, key) {
    const ret = {}
    for (const i of ["receive", "send"]) {
      const k = `snowbaby:count:${i}${key}`
      if (type == "keys") ret[i] = (await this.localStoreKeysLength(k)) || 0
      else ret[i] = (await localStore.get(k)) || 0
    }
    return ret
  }

  async localStoreKeysLength(MATCH) {
    let cursor = 0,
      length = 0
    do {
      const reply = await localStore.scan(cursor, { MATCH, COUNT: 10000 })
      cursor = reply.cursor
