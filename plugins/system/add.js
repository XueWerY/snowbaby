...
export class Add extends Plugin {
  ...
  /** 群号key */
  get grpKey() {
    return `snowbaby:group_id:${this.e.user_id}`
  }
  ...
  async getGroupId() {
    ...
    if (this.e.isGroup) {
      ...
      await localStore.setEx(this.grpKey, 2592000, String(this.group_id))
      return this.group_id
    }

    return (this.group_id = await localStore.get(this.grpKey))
  }
  ...
  trimAlias(msg) {
    const groupCfg = cfg.getGroup(this.e.self_id, this.group_id)
    const alias = Array.isArray(groupCfg.botAlias) ? groupCfg.botAlias : [groupCfg.botAlias]
