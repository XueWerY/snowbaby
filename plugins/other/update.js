...
export class Update extends Plugin {
  ...
  init() {
    ...
    if (cfg.bot.updateTime) this.autoUpdate()

    this.task = []
    if (cfg.bot.updateCron)
      for (const i of Array.isArray(cfg.bot.updateCron)
        ? cfg.bot.updateCron
        : [cfg.bot.updateCron])
        this.task.push({
          name: "定时更新",
  ...
  restart() {
    // 本项目不依赖 QQ 自动更新重启逻辑（restart.js 已移除）
  }
  ...
  async getLog(plugin = "") {
    ...
    const msg = [`${plugin || "snowbaby"} 更新日志，共${log.length}条`, log.join("\n\n")]
